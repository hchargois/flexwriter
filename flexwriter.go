...
// Column hold the configuration of a column for the flex writer. This interface
// is sealed, use either [Rigid] or [Flexed] to create Columns.
...
type Flexed struct {
	// Weight is the flex weight of the column; if 0 or less, it is set to 1.
	Weight int
	...
	// By default (if 0 or less), it is set to 10.
	Min int
	// Align is the alignment of the content within the column; default is left.
	Align Alignment
}
...
// (`\n`) and within a row the columns should be delimited by a tab (`\t`).
...
// Flush writes the contents of the internal buffer to the output.
